...
public final class RouteyServlet extends HttpServlet {
	...
	private void scanMappingAnnotations() throws IOException,
			ClassNotFoundException {
		for (final String aPackage : findPackagesToScan()) {
			AnnotationScanner.scanPackage(aPackage, Mapping.class,
					new AnnotationScannerNotifier() {

						@Override
						public void foundClass(final String packageName,
								final Class<?> classFound,
								final Method methodFound) {
							LOGGER.debug("found annotated method: "
									+ classFound.getName() + "."
									+ methodFound.getName() + "()");
							RequestMethod[] methods;

							final Mapping mapping = methodFound
									.getAnnotation(Mapping.class);
							if (mapping.value().length == 0) {
								LOGGER.error("Missing request URI in "
										+ classFound.getName() + "."
										+ methodFound.getName()
							...
							if (mapping.method().length == 0) {
								LOGGER.debug("Mapping " + classFound.getName()
										+ "." + methodFound.getName()
										+ "() to default GET method.");
							...
							for (final String requestUri : mapping.value()) {
								if (!initializedControllers
										.containsKey(classFound.getName())) {
									final Object controller = classFound
											.newInstance(); // TODO
									initializedControllers.put(
											classFound.getName(), controller);
								}
								for (final RequestMethod reqMethod : methods) {
									switch (reqMethod) {
									case DELETE:
										LOGGER.debug("map uri \"" + requestUri
												+ "\" --> "
												+ classFound.getName() + "."
												+ methodFound.getName()
												+ "() for request method "
												+ reqMethod);
										requestMappingsForDELETE.put(
												requestUri,
												new Routing(
														initializedControllers.get(classFound
																.getName()),
														methodFound));
										break;
									case GET:
	...
	protected void doDelete(final HttpServletRequest req,
		...
		final Routing routing = requestMappingsForDELETE.get(req
				.getRequestURI());
		if (routing != null) {
	...
	protected void doOptions(final HttpServletRequest req,
		...
		final Routing routing = requestMappingsForOPTIONS.get(req
				.getRequestURI());
		if (routing != null) {
			mapArgsForRoutingAndInvoke(req, resp, routing);
		} else {
	...
	protected void doTrace(final HttpServletRequest req,
		...
		final Routing routing = requestMappingsForTRACE
	...
	protected List<String> findPackagesToScan() {
		...
		final ServiceLoader<RouteyPackageScanner> serviceLoader = ServiceLoader
				.load(RouteyPackageScanner.class);

		for (final RouteyPackageScanner packageScanner : serviceLoader) {
			final Collection<String> addPackagesToScan = packageScanner
					.addPackagesToScan();
	...
	protected void mapArgsForRoutingAndInvoke(final HttpServletRequest req,
			final HttpServletResponse resp, final Routing routing) {
